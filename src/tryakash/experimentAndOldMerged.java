 ...
 * mapping.
 */
public class experimentAndOldMerged extends PApplet implements LLRPEndpoint, Observer {
	
//	public static HashMap<String, Integer> epcCountMap = new HashMap<String, Integer>();
    
	
	//RFID
	Location eicLocation = new Location(30.6235f,-96.347619f);
    Location riversideLocation = new Location(30.641602f , -96.4739f);
...
//    	 
    	 ...
    	 signCurrent = signStop;
      	 signNext = signYield;
      	 signNextNext = loadImage("tees.png");
...
//			                          }
        ...
        while(i.hasNext()){
          ...
          //  int zoomLevel = 15;
    	    map.zoomAndPanTo(zoomLevel, new Location(30.641602 , -96.4739)); //riverside
    	  ...
    	  //  ImageMarker imgMarkerLocation = new ImageMarker(eicLocation, loadImage("car.png"));
    	   // markerManager.addMarker(imgMarkerLocation);
...
//	} 
	...
	//test t1 = new test();
	  ...
	  ob.addObserver(new experimentAndOldMerged());
...
public static int enableGPSBreadCrumbs = 0;
public static String signValue = "stop";
public static String signValueNext = "stop";
public static String signValueNextNext = "stop";
	...
	public void draw() {
    	...
    	if(signValueNext.equalsIgnoreCase("street")){
    		image(signNext, 850+250,100, signNext.width, signNext.height );
    	}else{
    	...
    	if(signValueNextNext.equalsIgnoreCase("street")){
    		image(signNextNext, 850,400, signNextNext.width, signNextNext.height );
    	}else{
...
//     	}
    	...
    	if(STATUS_GPS_DRAW == 1 && enableGPSBreadCrumbs == 1){
    		
    		location =  new Location(Double.parseDouble(GPSLat),Double.parseDouble(GPSLong));
    		if(location == null){
    			location = eicLocation;
    		}
    		ImageMarker imgMarker1 = new ImageMarker(location, loadImage("car.png"));
...
//    			markerManager.addMarker(imgMarker1);           
//    		}else
    		{
    			//markerManager.removeMarker(imgMarker1);
    			markerManager.addMarker(imgMarker1);
            ...
            for (TagReportData tag : tags)
            {
            	...
            	// System.out.println("test: "+assetEPC.get(tmpTagPrint));
            	 if(printTagRFIDObj != null){
           		  ...
           		  String queueTmpString = readSignsOrder.toString();
           		  queueTmpString = queueTmpString.replace("[", "");
           		  queueTmpString = queueTmpString.replace("]", "");
           		queueTmpString = queueTmpString.replace(" ", "");
           		  String queueTmpArrayString[] = queueTmpString.split(",");
           		RFIDObj printTagRFIDObjNext = new RFIDObj();
          		 RFIDObj printTagRFIDObjNextNext = new RFIDObj();
          		 
           		    try{
           		    	
           		 if(queueTmpArrayString.length >= 3){
           		 System.out.println("Array####"+queueTmpArrayString[queueTmpArrayString.length-1]+","+queueTmpArrayString[queueTmpArrayString.length-2]+","
           		 +queueTmpArrayString[queueTmpArrayString.length-3]);
           		 printTagRFIDObjNext = lifeExpMap.get(queueTmpArrayString[queueTmpArrayString.length-2]);
        		 printTagRFIDObjNextNext = lifeExpMap.get(queueTmpArrayString[queueTmpArrayString.length-3]);//printTagRFIDObj;//lifeExpMap.get(readSignsOrder.get(readSignsOrder.size()-2));
        		
           		 } else if(queueTmpArrayString.length == 2)
           		 {
           			System.out.println("Array####"+queueTmpArrayString[queueTmpArrayString.length-1]+","+queueTmpArrayString[queueTmpArrayString.length-2]);
           			printTagRFIDObjNext = lifeExpMap.get(queueTmpArrayString[queueTmpArrayString.length-2]);
           		 
           			 
           		 }else if (queueTmpArrayString.length == 1){
            			System.out.println("Array####"+queueTmpArrayString[queueTmpArrayString.length-1]
                         		 );
            		 ...
            		 if(printTagRFIDObjNext == null)
            			 printTagRFIDObjNext = printTagRFIDObj;
            		 if(printTagRFIDObjNextNext == null)
            			 printTagRFIDObjNextNext = printTagRFIDObj;
            		 
            		 
            		 signValueNext = printTagRFIDObjNext.sign.substring(0,6);
            		 if(printTagRFIDObjNext.assetID!=51)
            			 signNext = loadImage(printTagRFIDObjNext.assetID+".png");
            		 signValueNextNext = printTagRFIDObjNextNext.sign.substring(0,6);
            		 if(printTagRFIDObjNextNext.assetID!=51)
            			 signNextNext = loadImage(printTagRFIDObjNextNext.assetID+".png");
            		 
            		// System.out.println("akash####"+printTagRFIDObjNext.assetID+",,,"+printTagRFIDObjNextNext.assetID);
            		 
            		 
            		 
            		 
            		// signValueNext = Integer.toString(readSignsOrder[4]);
